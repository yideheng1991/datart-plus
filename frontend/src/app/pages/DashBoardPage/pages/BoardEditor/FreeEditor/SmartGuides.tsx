 ...
 */
import { BoardScaleContext } from 'app/pages/DashBoardPage/components/FreeBoardBackground';
...
export const SmartGuides: React.FC = () => {
  ...
  useEffect(() => {
    const handleMove = (selectedIdStr: string, deltaX: number, deltaY: number) => {
      if (!draggingIdsRef.current.length) {
        draggingIdsRef.current = selectedIdStr
          .split(',')
          .filter(Boolean);
  ...
  return (
    <GuidesContainer>
      {guides.map((guide, idx) => (
        <Guide key={idx} orientation={guide.orientation} position={guide.position} />
      ))}
    </GuidesContainer>
  );
};
...
const Guide = styled.div<{ orientation: 'vertical' | 'horizontal'; position: number }>`
  position: absolute;
